...
[![Release](https://img.shields.io/github/v/release/ghauri01/OpsMaxx?label=download&sort=semver)](https://github.com/ghauri01/OpsMaxx/releases/latest)
...
| Platform | File | Notes |
|---|---|---|
| **Windows** | `OpsMaxx-x.y.z-setup.exe` | Installer, desktop + Start-menu shortcut |
| **Windows (portable)** | `OpsMaxx-x.y.z-portable.exe` | Single file, no install, keeps its data beside the `.exe` |
| **macOS** | `OpsMaxx-x.y.z-arm64.dmg` | Apple Silicon and Intel builds |
| **Linux** | `OpsMaxx-x.y.z-x64.AppImage` | `chmod +x` and run |
| **Linux (deb)** | `OpsMaxx-x.y.z-x64.deb` | `sudo apt install ./OpsMaxx-*.deb` |
