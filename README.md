...
[Features](#features) · [Install](#install) · [Quick start](#quick-start) · [Comparison](#opsmaxx-vs-mobaxterm-putty-termius-and-securecrt) · [Workspaces](#workspaces) · [Command palette](#command-palette) · [Shortcuts](#keyboard-shortcuts) · [Databases](#databases) · [Vault](#vault) · [AI & MCP](#ai--mcp) · [FAQ](#faq) · [Contributing](#contributing) · [Licence](#licence)
...
OpsMaxx is a **free and open-source alternative to MobaXterm, PuTTY, Termius, SecureCRT and MobaXterm Personal Edition**, built for engineers who spend the day moving between bastions, production boxes and databases — and increasingly, for AI coding agents that need to as well. It combines an **SSH terminal**, **SFTP file browser**, **server monitoring**, **SSH tunnels**, a **multi-engine database client**, an **encrypted password vault** and a **secure [MCP](#ai--mcp) bridge for Claude Code, Claude Desktop, Codex and other AI agents** in a single window — with no account, no telemetry and no subscription.
...
**OpsMaxx is not code-signed yet**, so the first time you run it you will see a
...
- Run once in Terminal:
  ```bash
  xattr -cr /Applications/OpsMaxx.app
  ```
...
<details>
<summary><b>macOS (Apple Silicon) — "OpsMaxx is damaged and can't be opened. You should move it to the Trash."</b></summary>
...
OpsMaxx: list servers, run commands, browse and edit files, check metrics. The agent never
sees an SSH password, private key or database credential. OpsMaxx resolves the server by
...
4. Point Claude Code, Claude Desktop, Codex or another MCP client at OpsMaxx's local MCP server with that token.
...
Every AI action OpsMaxx processed — allowed outright, approved, denied or failed — with the agent, workspace/server, the exact action and the result. Nothing here ever contains a password, key or other secret: command output and file paths are redacted before they are ever written to disk, not just before they are displayed.
...
- **Claude Code** — `claude mcp add --transport http opsmaxx http://127.0.0.1:<port>/mcp --header "Authorization: Bearer <token>"`
...
- **The `opsmaxx` CLI launcher** — skip copying tokens entirely: `opsmaxx claude` / `opsmaxx codex` pop a one-time pairing code inside OpsMaxx itself (the code is never sent back to whatever local process asked for it), and the session, token and MCP config are wired up automatically; `opsmaxx run -- <command>` does the same for any other MCP-aware CLI
...
None of that is how OpsMaxx's MCP bridge works, at every one of those steps:

- **The agent never gets a credential — or your IP, or a username.** It asks for
  "Production API" by name; OpsMaxx resolves the real host, port, username and key
...
Because OpsMaxx is not code-signed. Windows and macOS expect an application to carry a
...
**Open**, or run `xattr -cr /Applications/OpsMaxx.app` — on **Apple Silicon** this same cause
can show up as *"OpsMaxx is damaged and can't be opened"* instead; the fix is the same
...
Yes — see [AI & MCP](#ai--mcp). OpsMaxx runs a local [MCP](https://modelcontextprotocol.io)
...
anything marked ASK stops and waits for you to approve or deny it in OpsMaxx — an agent can
