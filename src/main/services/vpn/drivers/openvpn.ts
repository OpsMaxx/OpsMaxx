 ...
 *
 *  OpsMaxx never bundles it: OpenVPN is GPL-2.0 and OpsMaxx is Apache-2.0
...
function absentReason(e: unknown, platform: NodeJS.Platform): string {
  ...
  return `${detail} OpsMaxx does not include OpenVPN, because its licence and OpsMaxx's cannot be combined. ${installHint(platform)}`
}
