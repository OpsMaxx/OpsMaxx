...
const AUTH_MESSAGES: Record<AuthFailureReason, string> = {
  'ai-disabled': 'AI & MCP access is currently disabled in OpsMaxx.',
  ...
  revoked: 'This session has been revoked from OpsMaxx. Ask the user to create a new one.',
...
function buildServer(): McpServer {
  const server = new McpServer({ name: 'opsmaxx', version: '1.0.0' })

  server.registerTool(
    'list_workspaces',
    {
      description:
        "Lists the workspace this AI session is scoped to. OpsMaxx sessions are created for one workspace at a time; this never reveals workspaces outside the session's grant."
    },
    async (extra) => {
