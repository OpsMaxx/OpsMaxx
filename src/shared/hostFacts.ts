...
export const FACT_STATUS_HELP: Record<FactStatus, string> = {
  ...
  'stale-metadata':
    'The answer was read from a package cache that has not been refreshed recently, so it describes the estate as it was then, not now. OpsMaxx never refreshes it, because that is a network operation and on some package managers it can break the host.',
  unsupported:
