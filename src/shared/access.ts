...
function buildStagedWrite(o: {
  ...
  return [
    ...
    'for SP_OLD in "$HOME"/.ssh/*.opsmaxx-*.bak; do [ -e "$SP_OLD" ] || continue; echo "a key change staged earlier is still waiting for its rollback window to close ($SP_OLD); nothing was changed. If that window has already passed, its rollback did not run on this host: compare that file against the live authorized_keys and remove it by hand once you are satisfied." >&2; exit 6; done',
