...
export const ACCESS_WRITE_SCOPE =
  ...
  'OpsMaxx connects as on each host — not another account\u2019s file, and not any path sshd was ' +
  ...
  for (const t of req.targets) {
    ...
    if (req.kind === 'revoke') {
      ...
      if (protect.has(key)) {
        block(
          'is-session-key',
          `that key is the one this session is authenticated with on ${t.serverName}. Removing it would end OpsMaxx's own way back into the host, so it is refused here rather than confirmed anywhere.`
        )
        continue
      }
      ...
      if (t.access.sessionKeyFingerprint === null && t.access.collectedAs === t.user) {
        block(
          'session-key-unknown',
          `${t.serverName} does not report which key this session authenticated with (sshd's ExposeAuthInfo is off), and this would edit the keys of the very account OpsMaxx connects as. Without that fact nothing can prove the key being removed is not the one holding this connection open.`
        )
        continue
      }
