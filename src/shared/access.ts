...
function buildStagedWrite(o: {
  ...
  return [
    ...
    'for SP_OLD in "$HOME"/.ssh/*.opsmaxx-*.bak; do [ -e "$SP_OLD" ] || continue; echo "a key change staged earlier is still waiting for its rollback window to close ($SP_OLD); nothing was changed" >&2; exit 6; done',
