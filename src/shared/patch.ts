...
export function verifyReboot(
  ...
  if (before === null || before === 'unknown' || after.bootId === null) {
    ...
    return {
      ...
      ok: false,
      reason:
        'The host is answering again, but it does not expose a boot id, so OpsMaxx cannot ' +
...
export function planPatch(req: PatchPlanRequest): PatchPlan {
  ...
  for (const h of req.hosts) {
    ...
    if (h.packageManager === null) {
      const reason =
        'OpsMaxx has not identified a package manager on this host, so it cannot know what ' +
