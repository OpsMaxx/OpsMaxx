...
export function DockerPanel({ servers }: { servers: Server[] }): React.JSX.Element {
  ...
  const load = async (sudoOverride?: boolean): Promise<void> => {
    ...
    setLogs(null)
    try {
      const r = await window.opsmaxx?.docker?.list(cfgFor(server), { sudo: sudoOverride ?? useSudo })
      setProbe(r ?? null)
      ...
      if (r && !r.ok && r.reason === 'permission-denied') {
        const can = await window.opsmaxx?.docker?.canSudo(cfgFor(server))
  ...
  const openLogs = async (c: DockerContainer): Promise<void> => {
    ...
    try {
      const r = await window.opsmaxx?.docker?.logs(cfgFor(server), c.name, 200)
