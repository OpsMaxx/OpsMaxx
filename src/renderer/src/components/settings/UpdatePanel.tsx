...
export function UpdatePanel(): React.JSX.Element {
  ...
  useEffect(() => {
    void window.opsmaxx?.getVersion().then(setVersion)
    void window.opsmaxx?.updater.status().then(setStatus)
    const off = window.opsmaxx?.updater.onStatus(setStatus)
    return () => off?.()
  }, [])
    ...
    <div className="settings-section">
      ...
      <div className="setting-row">
        <div className="s-info">
          <div className="s-title">OpsMaxx {version ? `v${version}` : ''}</div>
        ...
        <button className="btn sm" disabled={checking} onClick={() => void window.opsmaxx?.updater.check()}>
      ...
      {status.state === 'downloaded' && (
        <div className="backup-banner ok">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="s-desc">OpsMaxx will restart to finish installing v{status.version}.</div>
          </div>
          <button className="btn sm primary" onClick={() => void window.opsmaxx?.updater.install()}>
            Restart &amp; update
          </button>
        </div>
      )}
      ...
      {status.state === 'manual' && (
        <div className="backup-banner warn">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div className="s-desc">
              macOS builds here are not notarized, so OpsMaxx cannot safely replace itself
