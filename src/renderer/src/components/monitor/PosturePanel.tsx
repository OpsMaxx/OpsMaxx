...
function certCell(posture: HostPosture): Cell {
  ...
  if (days === null) {
    ...
    return {
      ...
      help:
        s.status === 'absent'
          ? `None of the directories OpsMaxx looks in exists on this host, and that was checked rather than assumed. ${bound}`
...
const COLUMNS: { id: string; label: string; help: string }[] = [
  ...
  { id: 'updates', label: 'Security updates', help: 'Pending security updates as the host’s own package manager counts them. Collected by the Inventory probe and shown here unchanged — OpsMaxx computes nothing from a CVE feed.' },
    ...
    <div className="bc-panel">
      ...
      {summary.collected === 0 ? (
        <div className="s-desc">
          <b>No security posture has been collected yet.</b> OpsMaxx reads it about once an hour,
