...
function RealSftp({ server, tabId }: { server: Server; tabId?: string }): React.JSX.Element {
  ...
  useEffect(() => {
    return window.opsmaxx?.sftp.onExternalSaved((r) => {
  ...
  const list = useCallback(
    async (p: string): Promise<boolean> => {
      setLoading(true)
      setError(null)
      const res = await window.opsmaxx?.sftp.list(key, p)
      if (res?.ok && res.data) {
        setEntries(res.data)
        setPath(p)
        setLoading(false)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setLoading(true)
      const res = await window.opsmaxx?.sftp.connect(key, cfg())
      if (!alive) return
      if (res?.ok) {
  ...
  useEffect(() => {
    return window.opsmaxx?.sftp.onProgress((p) => {
