...
export function DatabaseView({ db }: { db: DatabaseConn }): React.JSX.Element {
  ...
  const loadInfo = useCallback(
    async (dbn: string): Promise<DbInfo | undefined> => {
      const i = await window.opsmaxx?.db.info(cfgWith(dbn))
      if (i) setInfo(i)
      return i
    },
    [cfgWith]
  ...
  const init = useCallback(
    async (dbn: string): Promise<void> => {
      ...
      const r = await window.opsmaxx?.db.test(cfgWith(dbn))
      if (!r?.ok) {
  ...
  const run = useCallback(async () => {
    ...
    setRunning(true)
    const r = await window.opsmaxx?.db.query(cfgWith(dbName), query)
