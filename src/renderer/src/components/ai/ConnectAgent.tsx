  ...
  const connect = async (target: Target, agentName: string): Promise<void> => {
    ...
    try {
      ...
      if (workspaces.length === 0) {
        throw new Error('Create a workspace in OpsMaxx first — an agent session is scoped to one.')
      }
      ...
      if (target === 'claude-code') {
        ...
        if (typeof write !== 'function') throw new Error(`Connecting ${label} needs a newer OpsMaxx build.`)
