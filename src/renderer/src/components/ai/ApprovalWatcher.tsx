...
export function ApprovalWatcher(): React.JSX.Element | null {
  ...
  useEffect(() => {
    void window.opsmaxx?.aiMcp.listApprovals().then((a) => setQueue(a ?? []))
    const off = window.opsmaxx?.aiMcp.onApprovalEvent((e) => {
  ...
  const respond = async (decision: 'approved' | 'denied'): Promise<void> => {
    await window.opsmaxx?.aiMcp.respondApproval(current.id, decision)
