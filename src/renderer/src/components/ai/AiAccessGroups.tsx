...
function ServerAssignment({ groups }: { groups: AccessGroup[] }): React.JSX.Element {
  ...
  const setWorkspaceGroup = async (groupId: string | null): Promise<void> => {
    await window.opsmaxx?.aiPolicy.setAssignment({ level: 'workspace', workspaceId: activeWorkspaceId }, groupId)
    load()
  }
  const setServerOverride = async (serverId: string, groupId: string | null): Promise<void> => {
    await window.opsmaxx?.aiPolicy.setAssignment({ level: 'server', serverId }, groupId)
    load()
  }
  const clearServerOverride = async (serverId: string): Promise<void> => {
    ...
    if (existing) await window.opsmaxx?.aiPolicy.removeAssignment(existing.id)
    load()
  }
...
export function AiAccessGroups(): React.JSX.Element {
  ...
  const deleteGroup = async (): Promise<void> => {
    if (!draft) return
    const result = await window.opsmaxx?.aiPolicy.deleteGroup(draft.id)
    if (result && !result.ok) {
