...
function remedyFor(
  ...
  switch (code) {
    ...
    // OpsMaxx, so a missing one is a damaged install and a download page
...
export function VpnManager(): React.JSX.Element {
  ...
  const stop = useCallback(async (p: VpnProfile, cancelling = false): Promise<void> => {
    setBusy((b) => ({ ...b, [p.id]: true }))
    const r = await window.opsmaxx?.vpn.stop(p.id)
    setBusy((b) => ({ ...b, [p.id]: false }))
    if (r?.ok) {
  ...
  const remove = useCallback(
    async (p: VpnProfile): Promise<void> => {
      await window.opsmaxx?.vpn.stop(p.id)
          ...
          <div className="col" style={{ gap: 6, padding: '6px 16px' }}>
            <div className="row" style={{ gap: 6, color: 'var(--danger)', fontSize: 11 }}>
              ...
              {/* Only OpenVPN is the user's to install or to point us at.
                  WireGuard and frp ship with OpsMaxx, so neither button
