...
| Biometric unlock key (opt-in, off by default) | `opsmaxx-vault-bio.json` | The vault's **derived key**, wrapped with Electron `safeStorage`. Present only if you enable Touch ID unlock. The master password is not stored, but see below — this is the weakest protection any vault data gets. |
