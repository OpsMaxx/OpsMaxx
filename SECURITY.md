...
| AI/MCP agent sessions | `opsmaxx-mcp-sessions.json` | Only a SHA-256 hash and a 4-character preview of the bearer token is stored — never the raw token. Also each session's permission mode, Bypass included. |
...
| MCP bridge settings | `opsmaxx-mcp-config.json` | Plaintext, `0600`. Whether the bridge is enabled, its port, session lifetime, and which access group and mode a new agent session starts on. No credential — but it is the configuration that decides what an agent is granted. |
