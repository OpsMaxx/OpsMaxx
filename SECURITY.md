...
| Servers, folders, workspaces; HTTP client collections, environments, globals and open tabs | `opsmaxx-data.json` | Sealed with the OS secure store (`safeStorage`). No SSH or database credentials, but every hostname, port, username and group — which is the estate, and worth sealing on its own. The HTTP client's data is here too, and it is **not** credential-free by construction: typed auth secrets (the Bearer, Basic and API-key fields, `Authorization`, `Proxy-Authorization` and `Cookie` header values, and URL passwords) are stripped before every save unless they are `{{variables}}` or vault references, but variables and other headers are saved as the user wrote them — an `X-Api-Key` header on a saved request or a token held in an environment variable is plaintext inside this sealed file, and syncs with the collection or environment when sync is set up. Open tabs and unsaved drafts are in this file too, with the same stripping: they never sync, but like everything here they are inside a backup bundle, encrypted under the backup's passphrase. The editors warn when such a value looks like a credential and offer to move it to the vault, and so does *Set as variable* on a response value. Not the vault's master password: the server list has to render while the vault is locked. On a machine with no OS keyring it is written in the clear, as it always was, and the diagnostics payload reports `dataFile.sealed: false`. |
| The previous copy of that file | `opsmaxx-data.json.bak` | Same. Written before each save so one bad write cannot lose the estate, and sealed with it — including on the migration that first seals the primary, since a sealed file beside a readable copy of itself is not sealed. |
| AI/MCP agent sessions | `opsmaxx-mcp-sessions.json` | Only a SHA-256 hash and a 4-character preview of the bearer token is stored — never the raw token. Also each session's permission profile, Bypass included. |
| AI/MCP audit log | `opsmaxx-ai-audit.jsonl` | Plaintext, append-only, `0600`. Free-text fields are passed through the same secret-redaction as command output before being written, so it should never contain a credential. Rows are **hash-chained** and the most recent one is pinned in the OS secure store, so an edit, a removal or a wholesale replacement is reported in the audit view rather than passing unnoticed. Tamper-EVIDENT, not tamper-proof: the pin is in the user's own keychain and anything running as the user can reach it. What it buys is that rewriting history is no longer editing a text file — it is editing a text file, recomputing a chain, and rewriting a keychain entry. Proof would mean shipping the rows off the machine, which is a different feature. |
...
| Debug trace (opt-in, off by default) | `opsmaxx-debug.jsonl` | Plaintext, append-only, `0600`, and **written only while Settings → Advanced → Debug mode is on** — deleted each time that switch is thrown, so it holds one reproduction rather than a history. Capped at 8 MB. Records which IPC operation ran, how long it took and whether it failed, plus main's own error output and errors raised in the UI. **Never the arguments to a call**, so passwords, passphrases and key material are absent by construction rather than by filtering; what is recorded still goes through the same secret redaction at the writer. It **does** contain hostnames, IPs, usernames, file paths and command text, which no rule can distinguish from ordinary words — which is why the app shows the whole report on screen before it will write it, and why a report is saved as an attachment rather than copied to the clipboard. Nothing is uploaded: you attach it to an issue yourself or not at all. |
| AI/MCP access-group policy | `opsmaxx-ai-policy.json` | Plaintext. Contains no credentials — capability rules, file-path patterns, each group's Confirm risky actions switch, and which workspaces and servers are Protected. |
...
| MCP bridge settings | `opsmaxx-mcp-config.json` | Plaintext, `0600`. Whether the bridge is enabled, its port, session lifetime, and which profile a new agent session starts on, plus the access group a Custom one uses. No credential — but it is the configuration that decides what an agent is granted. |
| The bridge entry written into an agent's own MCP config, and the `.opsmaxx-backup` copy taken before each write | `claude_desktop_config.json` under `~/Library/Application Support/Claude`, `~/.config/Claude` or `%APPDATA%\Claude`; `~/.codex/config.toml` | **A live bearer token, in plaintext**, `0600` — the backup copy too. Plaintext because the agent that launches the bridge has to read the token to present it; it is a session token, minted for named workspaces and a single permission profile, and not a credential for any host. These are the only files the app writes **outside its own data directory**, so the `0700` on that directory does not cover them and the mode is set on each file instead. The entry is merged into what was already there, and a config that cannot be parsed is left untouched rather than replaced. |
...
| HTTP client request history | `opsmaxx-http-history.jsonl` | One line per request, each sealed with `safeStorage`, and **written only when the OS offers a real keyring** — with none, or with Linux's `basic_text`, history is kept in memory for the session and never touches disk. Redacted structurally before it is stored, in the window and again in the main process: credential-named headers, query and form values and JSON keys (including a `value` beside a credential-named `name`), GraphQL literals under a credential-named argument, WebSocket subprotocols that carry a credential (anything but a plain protocol name, such as Kubernetes' `…authorization.k8s.io.<token>`), and the whole URL userinfo are masked unless they are `{{variables}}` or vault references, and response bodies, cookies and WebSocket frames are never stored. `0600`, appended through the same symlink- and owner-checked writer as the audit logs. Kept for 30 days or 2,000 entries, whichever comes first; a line that fails to decrypt is dropped. Never synced and never in a backup; *Clear history* and *Delete all data* remove it. |
...
| The addy account keys | the OS keychain, under `__machine__` names | **Key material, machine-bound.** Through `safeStorage`, so DPAPI on Windows and libsecret on Linux, and never in a file this app writes. Three per account: the device signing seed and the device encryption key, which are this machine's identity on the roster, and one account key per epoch — the one that opens every synced collection. They are `__machine__` names deliberately: such secrets are excluded from `exportSecrets` and refused by `importSecrets`, so they cannot leave in a backup bundle, which is what stops a bundle carrying the keys that open it. |
| Which addy relay this machine joined | `opsmaxx-addy.json` | Plaintext, `0600`. The relay address, the account id, the epoch, and the pinned keys this device verifies the roster against — **no key material**, all public halves. It is still a record of what this person belongs to, and the TLS pin is this machine's own view of that relay, which is why it is per-device and never synced. |
| What this machine has already synced | `opsmaxx-addy-sync.json` | Plaintext, `0600`. Per collection, the relay ETag it last matched, the highest counter it has seen and a SHA-256 of its own copy at that moment. No content and no key — a hash of a collection is not the collection — and it is this machine's own history, so syncing it would have every device adopt another's idea of what it had already done. |
| Files another of your devices sent here | `addy-transfers/` | **Plaintext file contents**, `0600`, written by a machine other than this one and kept in a directory this app owns rather than in Downloads. Swept after a week. Nothing here is ever opened automatically: "one of your own devices" is exactly the belief that would make that dangerous, since a device on the roster is a device somebody could have paired. |
