...
| Servers, folders, workspaces | `opsmaxx-data.json` | Sealed with the OS secure store (`safeStorage`). Contains no credentials, but every hostname, port, username and group — which is the estate, and worth sealing on its own. Not the vault's master password: the server list has to render while the vault is locked. On a machine with no OS keyring it is written in the clear, as it always was, and the diagnostics payload reports `dataFile.sealed: false`. |
