...
| **Incident response** | Inbox, ack, snooze, dedupe, flap damping, hysteresis, 400-day history; runbook per kind; rules alert → job | Failed unit → its log SHIPPED and now FILTERED (`shared/alertLogs.ts`); no systemd restart button; no rollback of anything; no DB restore; quarantine refused until 36; no incident span | 43, 44 |
