...
> **Not wired, and why — 6 Sep.** The module is complete and tested; what it has no path for is the COMMIT. A stored body lives in a vault entry, and `stageImportedSecrets` mints a NEW entry with a new id — so saving an edit means staging, committing, repointing the profile's `configRef` and disposing of the old entry, with an answer for the case where the renderer never saves and leaves an orphan. That is vault re-stage semantics, and inventing them at the end of a long session on the one subsystem where a mistake loses somebody's credentials is not a judgement call worth making quickly. The redaction, the restore and the four placeholder cases are the hard half and they are done. |
