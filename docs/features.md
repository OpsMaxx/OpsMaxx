...
| **Run one command everywhere** | Broadcast across selected servers with a confirmation naming exactly what runs where, per-server results, and a job that survives the app being closed. Save a job's steps as a template to reuse them — templates never remember servers, agents cannot read them, and they stay on this machine: they are not synced or included in a backup |
