...
- **No access group lets an AI agent start an frp profile** — only a session you have put in Bypass mode can — and starting any VPN asks for approval even when the access group allows it, unless that group's Confirm risky actions switch is off (Full Access ships with it off). See [AI-MCP.md](AI-MCP.md#permission-modes).
