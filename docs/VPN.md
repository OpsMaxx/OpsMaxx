...
- **Windows system mode needs `wintun.dll`**, which OpsMaxx does not bundle —
...
So you install OpenVPN yourself and OpsMaxx drives the copy you already
...
OpsMaxx looks in a fixed list of standard locations, and on POSIX also on
...
run. You can point at a specific binary explicitly; OpsMaxx records its path,
...
OpsMaxx asks per connect and never stores the answer:
...
| "The program that runs this tunnel could not be found" | For OpenVPN, install it. For WireGuard or frp, reinstall OpsMaxx — antivirus sometimes quarantines bundled binaries |
