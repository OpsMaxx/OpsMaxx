...
OpsMaxx is licensed under the [Apache License, Version 2.0](LICENSE). It
...
affected by OpsMaxx's own license.
...
|---|---|---|---|
| `opsmaxx-netd` | in-tree | Apache-2.0 | `sidecar/netd/` in this repository |
| `wireguard-go` (linked into `opsmaxx-netd`) | pinned in `sidecar/netd/go.mod` | MIT | `golang.zx2c4.com/wireguard` |
| gVisor `netstack` (linked into `opsmaxx-netd`) | pinned in `sidecar/netd/go.mod` | Apache-2.0 | `gvisor.dev/gvisor` |
...
direction. OpsMaxx supports OpenVPN by detecting a copy the user has already
...
binary would put OpsMaxx in the position of *distributing* GPL-2.0 software,
...
- An OpenVPN profile cannot connect until OpenVPN is installed. OpsMaxx says
  so, and says where to get it.
- OpsMaxx resolves the binary from a fixed allowlist of standard install
...
project ships. OpsMaxx does not include it; installing
