...
describe('the guided setup happens once', () => {
  ...
  it('says what the operator has to own, and then stops saying it', async () => {
    ...
    expect(
      await screen.findByText(/OpsMaxx does not host public addresses/)
    ).toBeTruthy()
    ...
    await waitFor(() =>
      expect(
        screen.queryByText(/OpsMaxx does not host public addresses/)
      ).toBe(null)
    )
    expect(
      await screen.findByText(
    ...
    expect(document.body.textContent).not.toContain('OpsMaxx does not host public addresses')
  })
