...
describe('path traversal (E37)', () => {
  ...
  it('refuses a symlink that leaves the import folder', () => {
    const tmp = mkdtempSync(join(tmpdir(), 'opsmaxx-ovpn-'))
    try {
...
describe('a benign profile passes through cleanly', () => {
  it('re-emits a minimal config we generated', () => {
    ...
    expect(r.ok).toBe(true)
    const config = body(r)
    expect(config.startsWith('# Generated by OpsMaxx')).toBe(true)
...
describe('a full real-world profile', () => {
  ...
  it('reports anything it does not recognise instead of swallowing it', () => {
    expect(dropped('nonsense-directive')).toBe('Not a setting OpsMaxx carries over.')
  ...
  it('carries only the allowlisted setenv names', () => {
    ...
    expect(config).toContain('setenv UV_PLATFORM opsmaxx')
  })
...
// OpsMaxx carries over". Every other drop makes the imported profile refuse
