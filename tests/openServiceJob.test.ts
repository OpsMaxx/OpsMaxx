...
describe('openServiceJob', () => {
  it('lands on the composer with the step filled in', () => {
    ...
    // Jobs moved to the Operations rail when the fleet destination was split by
    // surface: it composes work that runs on servers, and Monitoring's contract
    // is that nothing in it writes. Setting `monitorTab` here would leave the
    // jump filled in on a panel Monitoring no longer renders, so the operator
    // would land on Overview with a prefilled composer they cannot see.
  ...
  // The jump is only useful if the panel holding it is the one on screen.
  it('does not strand the prefilled step on a rail that is not showing', () => {
    openServiceJob('srv-1', 'restart', 'nginx.service')
    const { fleetRail, operationsTab, jobComposerJump } = useNav.getState()
    expect(jobComposerJump).not.toBeNull()
    expect(fleetRail).toBe('operations')
    expect(operationsTab).toBe('jobs')
  })
