...
// An RSA-3072 key, generated by `ssh-keygen -t rsa -b 3072` and fingerprinted
// by `ssh-keygen -l`. It is here for one number: `publickey ssh-rsa <blob>` is
// 562 characters, and the collector's general-purpose value cap is 512. Every
// key in this file above it is short enough to fit, which is exactly why the
// truncation bypass survived a full test suite.
const RSA3072 =
  ''
const RSA3072_FP = 'SHA256:OXxOrlGMdFmsuW/7HMAsHyS3HRCENji+f1cplKIZVD8'
...
interface FakeHost {
  ...
  collect: (opts?: { sudo?: boolean; hide?: string[]; env?: Record<string, string> }) => string
}
  ...
  return {
    ...
    collect: ({ sudo = false, hide = [], env = {} } = {}) => {
      ...
      return execFileSync('/bin/sh', ['-c', cmd], {
        encoding: 'utf8',
        // `env` is how SSH_AUTH_INFO_0 gets in. sshd puts it in the session
        // environment, so the only faithful way to test what the collector
        // makes of it is to put it in the environment the collector runs in.
...
describe.skipIf(process.platform === 'win32')('the collector, run against a host-shaped tree', () => {
  ...
  // -------------------------------------------------------------------------
  // SSH_AUTH_INFO_0 — which key is holding this connection open
  // -------------------------------------------------------------------------
  //
  // Run against the real environment variable rather than against a `V` record,
  // because the bug was in the shell: the value was cut at 512 characters by a
  // helper written for hostnames, and every key in the fixtures above happened
  // to be short enough to fit.

  it('names the key this session is on, and says it is sure', () => {
    const a = parse(host().collect({ env: { SSH_AUTH_INFO_0: `publickey ssh-ed25519 ${ED25519}` } }))
    expect(a.sessionKeyFingerprints).toEqual([ED25519_FP])
    expect(a.sessionKeysCertain).toBe(true)
  })

  it('names an RSA-3072 session key correctly rather than confidently wrongly', () => {
    // 562 characters. Cut at the old 512-character value cap the blob still
    // BASE64-DECODED — to a different key — so the fingerprint came out clean,
    // matched nothing in any authorized_keys file, and rule 1's exact check
    // silently did not fire on the one configuration where it is meant to be
    // exact.
    const a = parse(host().collect({ env: { SSH_AUTH_INFO_0: `publickey ssh-rsa ${RSA3072}` } }))
    expect(a.sessionKeyFingerprints).toEqual([RSA3072_FP])
    expect(a.sessionKeysCertain).toBe(true)
  })

  it('protects every key a multi-factor session presented, not just the first', () => {
    // `AuthenticationMethods publickey,publickey`. sshd puts one factor per
    // LINE in this one variable, and the collector's cleaner used to DELETE
    // newlines — flattening two factors into one corrupt record that parsed as
    // no key at all.
    const a = parse(
      host().collect({
        env: { SSH_AUTH_INFO_0: `publickey ssh-ed25519 ${ED25519}\npublickey ssh-rsa ${RSA3072}` }
      })
    )
    expect(a.sessionKeyFingerprints).toEqual([ED25519_FP, RSA3072_FP])
    expect(a.sessionKeysCertain).toBe(true)
  })
